      ...
      >
        ...
        <div className="p-8 bg-transparent shadow-none rounded-[2.5rem]">
          <div className="text-center mb-6">
            <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-accent/20 text-accent mb-4 shadow-xl shadow-accent/10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {isRegister && (
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-white/50">Nome Completo</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-white/50">E-mail</label>
              <div className="relative">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-white/50">Senha</label>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/50" />
                <Input
                  type={showPassword ? "text" : "password"}
                  className="pl-12 pr-12 bg-white/10 border-white/20 text-white placeholder:text-white/30 h-11 rounded-xl focus:ring-accent backdrop-blur-md"
                  placeholder="••••••••"
                  value={password}
            ...
            {isRegister && (
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest ml-1 text-white/50">Confirmar Senha</label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/50" />
                  <Input
            ...
            {error && (
              <motion.div 
                ...
                className="p-3 bg-destructive/20 border border-destructive/40 text-white rounded-xl text-xs font-medium backdrop-blur-md"
              >
                {error}
              </motion.div>
            )}
          ...
          <div className={`relative ${isRegister ? "my-10" : "my-6"}`}>
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-white/10" />
            </div>
            <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest">
              <span className={`px-4 bg-transparent text-white/50 ${isRegister ? "-mt-[10px]" : "-mt-[20px]"}`}>ou continue com</span>
            </div>
          </div>
          ...
          <div className="flex justify-center">
            <Button
              ...
              variant="outline"
              className={`w-full max-w-[200px] h-10 rounded-xl border-2 border-white/20 bg-white/5 backdrop-blur-md font-bold hover:bg-white/10 transition-all text-white text-xs ${isRegister ? "-mt-[20px]" : "-mt-[165px]"}`}
            >
